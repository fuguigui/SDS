...
```{r functions}
crabbingGlobal<-function(q, init_id, fdate, init_datetime, end_datetime){
  max_id = init_id
  last_time = init_datetime
  tweets<-NULL
  fname<-paste("./data/Tweet",fdate,"en.RData",sep="")
  
  while(strptime(last_time, "%Y-%m-%d %H:%M:%S") > strptime(end_datetime, "%Y-%m-%d %H")){
  ...
  keep_sign<-strptime(tweets$created_at, "%Y-%m-%d %H:%M:%S") > strptime(end_datetime, "%Y-%m-%d %H")
...
crabbingData<-function(n, queries, languages,since_ids,init_date, end_date){
  ...
  for(i in 1:n){
    ...
    keep_sign<-as.Date(tweets$created_at)>(as.Date(init_date,"%m%d")-1)
    tweets<-tweets[keep_sign,]
    save(tweets,file=fname)
  }
  return(last_ids)
...
id_hi = "1234626122836475904"
id_ko = "1234629518536495104"
id_ru = "1234629386395115533"
id_ja = "1234629551545704448"
id_de = "1234629554129506306"
id_it = "1234629522613403649"
...
init_datetime = "2020-03-01 23:59:52"
end_datetime = "2020-03-01 12"
q = "#COVID2019 OR #coronavirus"
fdate="0301"
crabbingGlobal(q,init_id,fdate, init_datetime, end_datetime)
```