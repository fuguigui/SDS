...
## Model on other sources

```{r}
short_paths<-read.csv(paste0(getwd(),"/data/Nodes/shortest_path.csv"))
it_row = short_paths[short_paths$X=="It",]
colnames(it_row)[it_row==1]

other = read.csv(paste0(path_s,files_s[14]),stringsAsFactors = F) # eu
data_other = other[18:nrow(other),2:ncol(other)] # 2.8
X_other<-scale(data_other)

test_data=xgb.DMatrix(data=X_other)
y_other_1 = predict(xgb, test_data)


other = read.csv(paste0(path_s,files_s[1]),stringsAsFactors = F) # eu
data_other = other[18:nrow(other),2:ncol(other)] # 2.8
X_other<-scale(data_other)

test_data=xgb.DMatrix(data=X_other)
y_other_2 = predict(xgb, test_data)

y_other = y_other_1+y_other_2
cor.test(y_other,y)
